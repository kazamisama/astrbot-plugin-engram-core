...
class DiaryStore:
    ...
    def add_line(self, line: DailyLine) -> DailyLine:
        ...
        c.execute(
            ...
            (line.id, line.channel_id, line.chat_type, line.actor_id,
             ...
             line.peer_name, line.session_id, line.platform))
        c.commit()
        return line

    def channels_with_lines(self, t0: float, t1: float) -> list:
        """Distinct channel_ids that have any line in [t0, t1)."""
        c = self._ensure_conn()
        rows = c.execute(
            "SELECT DISTINCT channel_id FROM daily_messages "
            "WHERE ts >= ? AND ts < ?", (t0, t1)).fetchall()
        return [r["channel_id"] for r in rows]

    def lines_in_range(self, channel_id: str, t0: float, t1: float) -> list:
    ...
    def find_idle_gap(self, channel_id: str, t0: float, t1: float,
                      min_gap_seconds: float) -> float | None:
    ...
    def add_chunks(self, chunks: list) -> int:
        ...
        for ch in chunks:
            c.execute(
                ...
                "text,embedding,embedding_model,ts_start,ts_end,created_at) "
                "VALUES(?,?,?,?,?,?,?,?,?,?)",
    ...
    def all_chunks(self, limit: int = 2000) -> list:
        c = self._ensure_conn()
        rows = c.execute(
