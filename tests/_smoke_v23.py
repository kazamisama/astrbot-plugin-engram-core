...
def test_registry_covers_all_memory_config_fields():
    ...
    assert len(registry_fields) == 122  # 67 base +5 backup +2 provider_id -4 openai +3 auto-inject +4 session-agg +3 persona +1 tokenizer +3 dedup +1 reltime +8 tiering +2 cold-archive +1 freq-recall +9 summary-b1 +1 idle-flush +1 summary-fallback +3 relation-b2 +11 diary-b3
...
def test_labels_have_zh_and_en_for_every_field():
    ...
    assert len(LABELS) == 122  # 67 base +5 backup +2 provider_id -4 openai +3 auto-inject +4 session-agg +3 persona +1 tokenizer +3 dedup +1 reltime +8 tiering +2 cold-archive +1 freq-recall +9 summary-b1 +1 idle-flush +1 summary-fallback +3 relation-b2 +11 diary-b3
