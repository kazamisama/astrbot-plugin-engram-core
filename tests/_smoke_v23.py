...
def test_registry_covers_all_memory_config_fields():
    ...
    assert len(registry_fields) == 80  # 67 base +5 backup +2 provider_id -4 openai +3 auto-inject +4 session-agg +3 persona
...
def test_labels_have_zh_and_en_for_every_field():
    ...
    assert len(LABELS) == 80  # 67 base +5 backup +2 provider_id -4 openai +3 auto-inject +4 session-agg +3 persona
